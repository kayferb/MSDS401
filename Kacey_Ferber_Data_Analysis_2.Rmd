---
title: "Ferber_Kacey Data Analysis Assignment #2 (75 points total)"
author: "Ferber, Kacey"
output:
...
mydata <- read.csv("~/Desktop/R/mydata.csv", sep = ",", stringsAsFactors = TRUE)
...
skew  <- rockchalk::skewness(mydata$RATIO)
skew
...
ggplot(mydata, aes(RATIO)) +
geom_histogram(color="black", fill=("aquamarine"),  size=.2, bins=30) +
...
skewlog  <- rockchalk::skewness(mydata$L_RATIO)
skewlog
klog  <- rockchalk::kurtosis(mydata$L_RATIO, excess = FALSE)
klog
klog2 <- rockchalk::kurtosis(mydata$L_RATIO)
klog2

ggplot(mydata, aes(L_RATIO)) +
geom_histogram(color="black", fill=("plum1"),  size=.2, bins=30) +
  ggtitle("Histogram of Log Ratio") +
   theme(plot.title = element_text(hjust = 0.5), legend.position="none", axis.title.x=element_blank()) +
  ylab("Frequency")


qqnorm(mydata$L_RATIO, main = "QQ plot of Log Ratio",
       col =  ("seagreen"))
qqline(mydata$L_RATIO)

ggplot(mydata, aes(y=L_RATIO, fill=CLASS)) +
  facet_grid(cols = vars(CLASS)) +
  geom_boxplot(color="black", size=.2, outlier.size=4, outlier.shape=4, fill="orchid1") + 
  ggtitle("Boxplot of L_RATIO differentiated by CLASS") +
...
analysis1 <- aov(L_RATIO ~ CLASS + SEX + CLASS*SEX, data=mydata)
summary(analysis1)

analysis2 <- aov(L_RATIO ~ CLASS + SEX, data=mydata)
summary(analysis2)
...
TukeyHSD(analysis1, conf.level=.95)
```
...
ggplot(mydata, aes(VOLUME, fill=TYPE)) +
  facet_grid(space="free", cols = vars(TYPE)) +
  geom_histogram(color="black", size=.2, bins=30) + 
  ggtitle("Histograms of Volume and Type") +
  theme(plot.title = element_text(hjust = 0.5))+
  ylab("Frequency") +
  xlab("Volume") 
```
...
shuckvolume <- ggplot(data = mydata, aes(x = VOLUME, y = SHUCK)) + 
  geom_point(aes(color = CLASS),size = 2) + 
  ggtitle(bquote('Shuck Weight v Volume')) +
  theme(plot.title = element_text(hjust = 0.5))+
  scale_color_brewer(palette="YlOrRd") +
  ylab("Shuck Weight") +
  xlab(bquote('Volume'))

lshucklvolume <- ggplot(data = mydata, aes(x = L_VOLUME, y = L_SHUCK)) + 
  geom_point(aes(color = CLASS),size = 2) + 
  ggtitle('L_Shuck Weight v L_Volume ') +
  theme(plot.title = element_text(hjust = 0.5))+
  scale_color_brewer(palette="YlOrRd") +
  ylab("L_Shuck Weight") +
  xlab('L_Volume')

shuckvolumetype <- ggplot(data = mydata, aes(x = VOLUME, y = SHUCK)) + 
...
lshucklvolumetype <- ggplot(data = mydata, aes(x = L_VOLUME, y = L_SHUCK)) + 
  geom_point(aes(color = TYPE),size = 2) + 
  ggtitle('L_Shuck Weight v  L_Volume') +
  theme(plot.title = element_text(hjust = 0.5))+
  scale_color_brewer(palette="Set1") +
...
grid.arrange(shuckvolume, lshucklvolume, shuckvolumetype, lshucklvolumetype, nrow=2)
...
sr  <- rockchalk::skewness(model$residuals)
sr
kr  <- rockchalk::kurtosis(model$residuals, excess = FALSE)
kr
kr2 <- rockchalk::kurtosis(model$residuals)
kr2


hist(model$residuals, col = "orchid1", xlab = "Residuals",
...
resclass <- ggplot(mydata, aes(x = L_VOLUME, y = RESIDUALS)) + 
  geom_point(aes(color = CLASS),size = 1) + 
  ggtitle("Residuals v L_Volume by Class") +
  scale_color_brewer(palette='Spectral') +
  theme(plot.title = element_text(hjust = 0.5))+
  ylab("Residuals") +
  xlab("L_Volume")

restype <- ggplot(mydata, aes(x = L_VOLUME, y = RESIDUALS)) + 
  geom_point(aes(color = TYPE),size = 1) + 
  ggtitle("Residuals v L_Volume by type") +
  scale_color_brewer(palette='Spectral') +
  theme(plot.title = element_text(hjust = 0.5))+
  ylab("Residuals") +
  xlab("L_Volume")

boxclass <- ggplot(mydata, aes(y=RESIDUALS, fill=CLASS)) +
  geom_boxplot(color="black", size=.2, outlier.size=2, outlier.shape=1) + 
  scale_color_brewer(palette='Spectral') +
  ggtitle("Boxplot of residuals colored by class") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Class") +
  ylab("Residuals")

boxtype <- ggplot(mydata, aes(y=RESIDUALS, fill=TYPE)) +
  geom_boxplot(color="black", size=.2, outlier.size=2, outlier.shape=1) + 
  scale_color_brewer(palette='Spectral') +
  ggtitle("Boxplot of residuals colored by type") +
  theme(plot.title = element_text(hjust = 0.5))+
  xlab("Type") +
  ylab("Residuals")

grid.arrange(resclass, restype, boxclass, boxtype, nrow=2)
...
maxinfvol <- max(mydata$VOLUME[mydata$TYPE == "I"])  # [1] 526.6383

# Proportion of adults harvested
a1 <- sum(mydata$VOLUME[mydata$TYPE == "ADULT"] > maxinfvol) /
  total.adults  # [1] 0.2476573

# Add code to calculate the proportion of infants harvested
i1 <- sum(mydata$VOLUME[mydata$TYPE == "I"] > maxinfvol) /
  total.infants # 0
...
# Add code to calculate the proportion of infants harvested
i2 <- sum(mydata$VOLUME[mydata$TYPE == "I"] > medinfvol) /
  total.infants
## 0.4982699

# Add code to calculate the proportion of adults harvested

a2 <- sum(mydata$VOLUME[mydata$TYPE == "ADULT"] > medinfvol) /
  total.adults
## 0.9330656
...
medadvol <- median(mydata$VOLUME[mydata$TYPE == "ADULT"])

# Add code to calculate the proportion of infants harvested
i3 <-  sum(mydata$VOLUME[mydata$TYPE == "I"] > medadvol) /
  total.infants
...
a3 <- sum(mydata$VOLUME[mydata$TYPE == "ADULT"] > medadvol) /
  total.adults
...
ggplot() + 
  ...
  geom_vline(xintercept = maxinfvol, linetype=1) +
  geom_hline(yintercept = 0.5, linetype = 2)+
  geom_vline(xintercept = medinfvol, linetype=1) +
  geom_vline(xintercept = medadvol, linetype=1) +
  annotate("text", label=paste("Median Infant = ",round(medinfvol, 2)), x=medinfvol+10, y=.5, angle=90) +
  annotate("text", label=paste("Median Adult = ",round(medadvol, 2)), x=medadvol+10, y=.5, angle=90) +
  annotate("text", label=paste("Largest Infant = ",round(maxinfvol, 2)), x=maxinfvol+10, y=.5, angle=90) +
  ggtitle("Proportion of Adults and Infants Protected") +
  theme(plot.title = element_text(hjust = 0.5))+
  xlab("Volume") +
  ylab("Proportion") +
  scale_color_brewer(palette='Paired', name="Legend", labels=c("Proportion of Infants", "Proportion of Adults"))
...
ggplot() + theme_bw() +
  geom_line(aes(volume.value, difference), col = 'seagreen') +
  geom_vline(xintercept = medinfvol) +
  annotate("text", label = paste('infant volume =', round(medinfvol, 2)), 
           x = medinfvol + 20, y = 0.2, angle = 90) +
  geom_vline(xintercept = medadvol) +
  annotate("text", label = paste('adult volume =', round(medadvol, 2)), 
           x = medadvol + 20, y = 0.2, angle = 90) +
  labs(x = "Volume", y = "Difference in Proportions Harvested", 
       title = "Harvest Proportions") + 
  theme(plot.title = element_text(hjust = 0.5))
...
ggplot() + 
  geom_line(aes(volume.value, difference), col = 'turquoise1') +
  geom_line(aes(volume.value, smooth.difference), col = 'orangered1') +
...
ggplot() + 
  ...
  geom_vline(xintercept = maxinfvol, linetype=2) +
  geom_vline(xintercept = medinfvol, linetype=2) +
  geom_vline(xintercept = medadvol, linetype=2) +
  geom_vline(xintercept = max, linetype=2) +
  annotate("text", label=paste("Median Infant = ",round(medinfvol, 2)), x=medinfvol+30, y=.25, angle=90) +
  annotate("text", label=paste("Median Adult = ",round(medadvol, 2)), x=medadvol+30, y=.25, angle=90) +
  annotate("text", label=paste("Largest Infant = ",round(maxinfvol, 2)), x=maxinfvol+30, y=.25, angle=90) +
...
a4 <- (1 - prop.adults)[which.max(smooth.difference)]  # [1] 0.7416332
# OR,
sum(mydata[mydata$TYPE == "ADULT", "VOLUME"] >
      volume.value[which.max(smooth.difference)]) / total.adults # [1] 0.7416332
i4 <- (1 - prop.infants)[which.max(smooth.difference)]
i4
...
zeroa1inf <- volume.value[volume.value > max(mydata[mydata$CLASS == "A1" &
  mydata$TYPE == "I", "VOLUME"])][1] # [1] 206.786
zeroa1inf

i5 <- sum(mydata[mydata$TYPE == "I", "VOLUME"] > zeroa1inf) / total.infants
i5 # [1] 0.2871972

a5 <- sum(mydata[mydata$TYPE == "ADULT", "VOLUME"] > zeroa1inf) / total.adults
a5 # [1] 0.8259705
```
...
ggplot() + 
  ...
  geom_hline(yintercept = 0.5, linetype = 2)+
  geom_vline(xintercept = medadvol, linetype=1) +
  geom_vline(xintercept = max, linetype=1) +
  geom_vline(xintercept = zeroa1inf, linetype=1) +
  annotate("text", label=paste("Median Infant = ",round(medinfvol, 2)), x=medinfvol+10, y=.25, angle=90) +
  annotate("text", label=paste("Median Adult = ",round(medadvol, 2)), x=medadvol+10, y=.25, angle=90) +
  annotate("text", label=paste("Largest Infant = ",round(maxinfvol, 2)), x=maxinfvol+10, y=.25, angle=90) + 
  annotate("text", label=paste("Maximum Difference = ",round(max, 2)), x=max+10, y=.35, angle=90) +
  annotate("text", label=paste("Zero A1 Inf = ",round(zeroa1inf, 2)), x=zeroa1inf+10, y=.25, angle=90) +
  ggtitle("Adults and Infants Protected") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Volume") +
  ylab("Proportion") +
  scale_color_brewer(palette='Paired', name="Legend", labels=c("Proportion of Infants", "Proportion of Adults")) 
...
ggplot() + 
  geom_line(aes(1-prop.infants, 1-prop.adults), color= 'seagreen') +
  geom_abline(slope=1, intercept=0, linetype=2) +
  ggtitle("ROC Curve of Adults vs Infants Harvested") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("1-prop.infants") +
  ylab("1-prop.adults") +
  geom_point(aes(x=i1, y=a1), size=3, shape=1) +
  geom_point(aes(x=i2, y=a2), size=3, shape=1) +
  geom_point(aes(x=i3, y=a3), size=3, shape=1) +
  geom_point(aes(x=i4, y=a4), size=3, shape=1) +
  geom_point(aes(x=i5, y=a5), size=3, shape=1) +
  annotate("text", label=paste("Protect all Infants = ",round(maxinfvol, 2)), x=i1+.2,y=a1) +
  annotate("text", label=paste("Median Infant = ",round(medinfvol, 2)), x=i2+.2, y=a2) +
  annotate("text", label=paste("Median Adult = ",round(medadvol, 2)), x=i3+.2, y=a3) +
  annotate("text", label=paste("Maximum Difference = ",round(max, 2)), x=i4+.2, y=a4) +
  annotate("text", label=paste("Zero A1 Inf = ",round(zeroa1inf, 2)), x=i5+.2, y=a5)
...
```{r Part_10} 	
t1 <- (a1*total.adults + i1*total.infants)/(total.adults+total.infants)
t2 <- (a2*total.adults + i2*total.infants)/(total.adults+total.infants)
t3 <- (a3*total.adults + i3*total.infants)/(total.adults+total.infants)
t4 <- (a4*total.adults + i4*total.infants)/(total.adults+total.infants)
t5 <- (a5*total.adults + i5*total.infants)/(total.adults+total.infants)


cut1 <- c(maxinfvol, a1, i1, t1)
cut2 <- c(medinfvol, a2, i2, t2)
cut3 <- c(medadvol, a3, i3, t3)
cut4 <- c(max, a4, i4, t4)
cut5 <- c(zeroa1inf, a5, i5, t5)
cut <- rbind(round(cut1, 2), 
         round(cut2, 2), 
         round(cut3, 2), 
         round(cut4, 2), 
         round(cut5, 2))
colnames(cut) <- c("Volume", "True Positive", "False Positive", "total Harvest")
rownames(cut) <- c("Protect All Infants", "Median Infants", "Median Adults", "Max Difference", "Zero A1 Infants")
kable(cut)
